...
int cAssignExt(int16_t c, int16_t D, int16_t AM) {
  ...
  switch (c) {
    ...
    default:
      sprintf(msg, "HackAlu + HackAluExt do not support c=0x%x", c);
      error(msg);
  }
  return out;
}
...
// 處理 C 型指令
int cInstr(int16_t e, int16_t a, int16_t c, int16_t d, int16_t j) {
  int AM = (a == 0) ? A : 
           (e == 0) ? m[A] : im[A];
...
int run(uint16_t *im, int16_t *m, int imTop) {
  ...
  while (1) {
    ...
    if ((I & 0x8000) == 0) { // A 指令
      ...
      e = (I & 0x6000) >> 13; // 透過 e 欄位決定延伸架構
      a = (I & 0x1000) >> 12;
      c = (I & 0x0FC0) >>  6;
      d = (I & 0x0038) >>  3;
      j = (I & 0x0007) >>  0;
      switch (e) {
        case 0x00: cInstr(e, a, c, d, j); break;
        case 0x01: fInstr(c, F); break;
        default: error("e=11 is reserved for extension!");
      }
    }
